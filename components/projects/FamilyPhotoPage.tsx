...
const FamilyPhotoPage: React.FC<{ navigateTo: (page: Page) => void }> = ({ navigateTo }) => {
  const [family, setFamily] = useState('a family of four (father, mother, son, daughter)');
  const [setting, setSetting] = useState('in a beautiful park during autumn');
  const [style, setStyle] = useState('candid and happy');
  ...
  const handleGenerate = async () => {
    ...
    if (uploadedImages.length > 0) {
      basePrompt = `Using the uploaded images as a reference for the people's appearance, create a beautiful, heartwarming photo of ${family}, ${setting}. The style is ${style}, with natural light. Photorealistic.`;
    } else {
      basePrompt = `A beautiful, heartwarming photo of ${family}, ${setting}. The style is ${style}, with natural light. Photorealistic.`;
    }
  ...
  const getImagesForDownload = (): ImageInfo[] => {
    return results
      ...
      .map((res, index) => ({
        url: res.imageUrl!,
        filename: `family-photo-${index + 1}.png`
      }));
  };
  ...
  return (
    <ProjectPageLayout
      title="Foto Keluarga"
      description="Ciptakan potret keluarga yang hangat dan tak terlupakan."
      navigateTo={navigateTo}
    >
      <div className="space-y-4 mb-6">
        <div>
          <h3 className="text-lg font-semibold text-slate-800 mb-2">Unggah Gambar Referensi (Opsional)</h3>
          <p className="text-sm text-slate-500 mb-3">Unggah foto anggota keluarga untuk kemiripan yang lebih baik.</p>
          <ImageUploader images={uploadedImages} onImagesChange={setUploadedImages} maxImages={4} />
        </div>
        <div>
          <label htmlFor="family" className="block text-sm font-medium text-slate-700 mb-1">Deskripsi Keluarga</label>
          <input id="family" type="text" value={family} onChange={(e) => setFamily(e.target.value)} className="w-full p-2 border border-slate-300 rounded-md bg-white" placeholder="Contoh: keluarga dengan ayah, ibu, dan dua anak" />
        </div>
        <div>
          <label htmlFor="setting" className="block text-sm font-medium text-slate-700 mb-1">Latar</label>
          <input id="setting" type="text" value={setting} onChange={(e) => setSetting(e.target.value)} className="w-full p-2 border border-slate-300 rounded-md bg-white" placeholder="Contoh: di taman yang indah saat musim gugur" />
        </div>
        <div>
          ...
          <select id="style" value={style} onChange={(e) => setStyle(e.target.value)} className="w-full p-2 border border-slate-300 rounded-md bg-white">
            <option>Candid and happy</option>
            <option>Formal studio portrait</option>
            <option>Lifestyle at home</option>
            <option>Vintage black and white</option>
          </select>
        </div>
      ...
      <button onClick={handleGenerate} disabled={loading} className="w-full bg-primary text-white font-bold py-3 px-4 rounded-lg hover:bg-primary-dark transition-colors disabled:bg-slate-400">
        {loading ? 'Memotret...' : 'Buat Foto Keluarga'}
      </button>
      ...
      <div className="mt-8">
        ...
        {results.length > 0 && (
          <div>
            <DownloadController getImages={getImagesForDownload} projectName="picaa-family-photo" />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
